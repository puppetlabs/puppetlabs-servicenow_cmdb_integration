...
class Mockserver
  ...
  def set(path, return_body = nil, query_params = {})
    data = {
      httpRequest: {
        path: normalize_path(path), # Turn the path into a regex that is less sensitive to presence or absence of a trialing slash.
      },
      httpResponse: {
        body: return_body,
      },
    ...
    # Mock server can match mocks based on query parameters along with the path. If the params are provided they are
    # Properly added to the request in this function.
    ...
    # However, this will also reset the counter for calls to assert_mock_called
    clear(path)
  ...
  def mock_called?(path, query_params = {}, atLeast = nil, atMost = nil)
    ...
    when 202
      true
    when 406
      matches = reply[:body].match(%r{(?<message>Request.+times)})
      raise matches[:message]
    else
