...
describe 'servicenow' do
  ...
  context 'checking correct configuration in ServiceNow object' do
    it 'creates servicenow helper with correct parameters' do
      ...
      expect(ServiceNowRequest).to receive(:new).with(uri, 'Get', nil, 'admin', 'password')
  ...
  context 'loading ServiceNow config' do
    ...
    context 'with hiera-eyaml encrypted password' do
      ...
      let(:config) do
        ...
        # 'eyaml encrypt -s 'password' --pkcs7-private-key=./spec/support/files/private_key.pkcs7.pem --pkcs7-public-key=./spec/support/files/public_key.pkcs7.pem'
      ...
      before(:each) do
        hiera_eyaml_config = {
          pkcs7_private_key: File.absolute_path('./spec/support/files/private_key.pkcs7.pem'),
          pkcs7_public_key: File.absolute_path('./spec/support/files/public_key.pkcs7.pem'),
        }
...
lqsUgBAYxyFLFLpVsGxI4XLR8hxD]
        ...
        it 'decrypts the password' do
          expect(ServiceNowRequest).to receive(:new).with(anything, anything, anything, 'admin', 'password')
          expect { servicenow('example.puppet.com') }.to raise_error(NoMethodError)
        end
      end
